...
export default function ContactForm() {
  ...
  return (
    <form className="mt-20 text-black text-3xl w-full" onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        className="bg-white rounded-2xl mt-2 py-4 px-6 w-full"
        onChange={updateName}
        value={name}
      />
      ...
      <textarea
        className="bg-white rounded-2xl mt-2 w-full py-4 px-6 h-50"
        onChange={updateDetails}
        value={details}
      />
      <button
        type="submit"
        className={`flex text-white bg-blue-700 rounded-4xl w-full mt-10 cursor-pointer py-3 justify-center justify-self-center select-none ${
