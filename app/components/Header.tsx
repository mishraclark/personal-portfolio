...
const Header: React.FC<{ mode?: "light" | "dark" }> = ({ mode = "dark" }) => {
  ...
  return (
    <div
      className={`flex z-[1000] top-0 max-md:inset-x-5 max-[1400px]:inset-x-16 min-[1400px]:inset-x-32 justify-between items-center border-zinc-500 ${scrollY > headerOffset && "fixed bg-slate-950 border-b border-l border-r"} px-5 py-3 rounded-b-lg transition-all duration-300`}
    >
      <div onClick={scrollToTop} className="cursor-pointer">
        <div className="flex flex-col gap-y-2 items-center">
          <img src={portraitIcon} className="size-10" />
          <span
            className={`max-sm:hidden text-3xl font-bold ${mode == "dark" ? "text-white" : "text-slate-700"}`}
          >
            Mishra
          </span>
        </div>
      ...
      <div
        className={`flex gap-6 md:gap-10 font-semibold md:text-lg ${mode == "dark" ? "text-white" : "text-slate-700"}`}
      >
