...
export default function Portfolio() {
  ...
  const portfolioItems = [
    ...
    },
    {
      ...
      details:
        "River Lyons is a very talented tattoo artist in the Portland area who wanted a portfolio webiste to help him network and get in touch with clients. The client's budget was tight, he wanted to be able to maintain and improve his site without assitance, but he didn't know what platform to choose or how to get started. I recommended WordPress, which is low cost and simple to learn. I built and launched an mvp of the site and provided training to ensure River would be able to maintain his site moving forward.",
    ...
    <div className="flex flex-col relative p-10 mt-10 lg:mt-[120px] border border-zinc-500 rounded-2xl overflow-hidden">
      ...
      <div className="flex-col z-30">
        <div className="flex justify-between items-center mb-10">
          <div
            id="portfolio"
            className="flex text-white text-2xl sm:text-3xl md:text-6xl font-bold"
          >
            Recent Work
          </div>
          ...
          <div className="flex items-center gap-x-6 lg:mr-20">
            <ArrowLeft
              className={`cursor-pointer ${selectedItem > 0 ? "size-8 md:size-11 text-amber-200" : "text-zinc-500 size-6 md:size-8 pointer-events-none"}`}
              onClick={previousSlide}
            />
            <ArrowRight
              className={`cursor-pointer  ${selectedItem < 4 ? "size-8 md:size-11 text-amber-200" : "text-zinc-500 size-6 md:size-8 pointer-events-none"}`}
              onClick={nextSlide}
            />
          </div>
        </div>
        ...
        <div className="flex relative w-full h-180 mt-16">
          ...
          {portfolioItems[selectedItem].link && (
            <div className="flex absolute bottom-0 left-0 right-0 justify-center">
              <a
                href={portfolioItems[selectedItem].link}
                className="cursor-pointer mt-10 px-10 py-4 text-lg text-amber-200 border-amber-200 border rounded-xl"
